...
- **Update `Verified Against`** — set to the REDCap version the edit was checked against, naming both lines (e.g. `REDCap v17.4.1 (Standard) / v17.3.7 (LTS)`). Only bump it if you actually verified the content against that version or a changelog diff for it; leaving an honest older value is better than an unchecked current-looking one. If the article has no `Requires` / `Verified Against` rows, add both — see the kb-creation skill's **Version tracking fields** section.
- **Set `Requires` only if the article's scope changed.** It records the earliest REDCap version supporting the documented feature and does not move on routine edits. `Applies To` never carries a version — migrate any legacy version string out of it into these fields.
- **If the edit documents behaviour that was wrong for a version range**, add a `> **Version caveat:**` blockquote immediately after the affected content — see the kb-creation skill's **Version caveats** section for format and the bar an entry must clear.
- **Never add an image, screenshot, or image placeholder**, and remove any you find. The KB is text-only: images are invisible to RAG retrieval and go stale as REDCap's UI changes. Describe the interface in words instead — exact labels in bold, location on the page, options in a table. See the kb-creation skill's **No images** section. Note that `@PLACEHOLDER` is a REDCap action tag and legitimate content; never bulk-remove on the string "PLACEHOLDER" alone.
