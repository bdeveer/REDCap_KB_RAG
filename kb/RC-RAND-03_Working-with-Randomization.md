...
**8. Questions & Answers**
  ...
  ----------------------------------------------------------------------------------------------------------------- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  *A user says they cannot click the Randomize button. What is the most likely cause?*                              The user does not have the Randomize user right assigned for this project. Check their user rights in Applications → User Rights. Note that this right is project-specific, not global.
  *The randomization popup appeared but the record did not get randomized after clicking Confirm. What happened?*   Check whether the project has run out of allocation slots for that group/stratum combination. If the \'Not Used\' count in the dashboard is 0, an administrator needs to append more allocations.
  *Can a record be randomized more than once?*                                                                      No. Once a record is randomized, the Randomize button is replaced by the randomization result and cannot be clicked again. Only an administrator can remove a randomization assignment.
  *A record was randomized to the wrong group due to a data entry error in a stratification variable. What now?*    Contact your REDCap administrator. The administrator can remove the randomization from the record (freeing the slot), correct the stratification variable, and allow re-randomization. This should be documented as a protocol deviation per your study\'s SOPs.
  *How do I know when I am running low on allocation slots?*                                                        Monitor the \'Not Used\' column in the randomization dashboard regularly. When a group approaches zero, contact your administrator to append more allocations before the slots are exhausted.
  *I can see the randomization dashboard but my colleague cannot. Why?*                                             The Dashboard user right is required to access the randomization dashboard. Check your colleague\'s user rights for this project and add the Dashboard right if appropriate.
  *What is the difference between the randomization dashboard and a project dashboard?*                             They are completely separate features. The randomization dashboard (in Applications → Randomization) shows allocation usage for a specific randomization model. Project dashboards (in Applications → Project Dashboards) are custom aggregate data views you build yourself.
  *Can an administrator see the group assignments in a blinded randomization?*                                      Through the allocation-level drill-down in the dashboard, an administrator can see the full allocation table including the target field and alternate values. This is a potential unblinding risk. Blinded study protocols should define who is allowed administrator access to a blinded REDCap project.
  *What does \[rand-time\] capture if the user is in a different time zone than the server?*                        \[rand-time\] always uses server time, not the user\'s local device time. For cross-timezone consistency, use \[rand-utc-time\] instead.
  *How do I find out which records have been randomized to each group?*                                             The randomization dashboard lists all assigned record IDs per group row. IDs are clickable and navigate directly to the record. For a more structured export, use a custom report in Data Exports, Reports, and Stats that includes the randomization variable.
  ----------------------------------------------------------------------------------------------------------------- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
